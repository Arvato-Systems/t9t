...
public class SqlDependencyHelper {
     ...
     */
    public List<String> getJarList(MavenProject project, List<SqlArtifact> sqlArtifacts) throws MojoExecutionException {
        ...
        for (SqlArtifact sqlArtifact : sqlArtifacts) {
            ...
            jarList.add(artifact.getFile().getPath());
        }
     ...
     * @throws MojoExecutionException if an {@link Artifact} was not resolved or its file is {@code null}
     */
    public List<String> getJarList(MavenProject project) throws MojoExecutionException {
        Set<Artifact> artifacts = getArtifactsWithSqlClassifier(project);
        List<String> jarList = new LinkedList<>();
