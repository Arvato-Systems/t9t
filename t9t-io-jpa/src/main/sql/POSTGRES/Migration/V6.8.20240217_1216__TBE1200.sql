-- TBE-1200 - new columns in p42_cfg_data_sinks (POSTGRES)

ALTER TABLE p42_cfg_data_sinks ADD COLUMN buffer_size integer;
ALTER TABLE p42_cfg_data_sinks ADD COLUMN record_size integer;

ALTER TABLE p42_his_data_sinks ADD COLUMN buffer_size integer;
ALTER TABLE p42_his_data_sinks ADD COLUMN record_size integer;

COMMENT ON COLUMN p42_cfg_data_sinks.buffer_size IS 'used for low level buffering (only if supported by the application, about 64 KB if unspecified)';
COMMENT ON COLUMN p42_cfg_data_sinks.record_size IS 'arbitrary field - format specific (size of a record for fixed formats, or upper bound to size in general)';
