...
public class LeanCsvConfigurationSearchRequestHandler extends AbstractLeanSearchRequestHandler<LeanCsvConfigurationSearchRequest, CsvConfigurationEntity> {
    public LeanCsvConfigurationSearchRequestHandler() {
        super(Jdp.getRequired(ICsvConfigurationEntityResolver.class), (final CsvConfigurationEntity it) -> {
            final String description = it.getDescription() == null ? "?" : it.getDescription();
            return new Description(null, it.getCsvConfigurationId(), description, false, false);
        });
    }
}
