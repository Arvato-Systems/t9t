...
public class LeanUserSearchRequestHandler extends AbstractLeanSearchRequestHandler<LeanUserSearchRequest, UserEntity> {
    ...
    public LeanUserSearchRequestHandler() {
        super(Jdp.getRequired(IUserEntityResolver.class), (user) -> {
            return new Description(null, user.getUserId(), user.getName(), false, false);
        });
    }
}
