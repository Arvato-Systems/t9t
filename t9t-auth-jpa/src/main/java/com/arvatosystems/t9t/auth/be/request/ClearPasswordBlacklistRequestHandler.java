...
public class ClearPasswordBlacklistRequestHandler extends AbstractRequestHandler<ClearPasswordBlacklistRequest> {

    private final IAuthPersistenceAccess authPersistenceAccess = Jdp.getRequired(IAuthPersistenceAccess.class);

    @Override
    public ServiceResponse execute(final RequestContext ctx, final ClearPasswordBlacklistRequest rq) {
        authPersistenceAccess.deletePasswordBlacklist();
        return ok();
    }
}
