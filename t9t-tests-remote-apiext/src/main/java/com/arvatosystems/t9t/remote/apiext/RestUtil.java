...
public final class RestUtil {
    ...
    @Nullable
    private static ByteArray toByteArray(@Nullable final Object payload, final RESTConnection connection) {
        if (payload == null) {
            return null;
        }
        if (payload instanceof BonaPortable bon) {
            try {
                ...
                LOGGER.error("Failed to marshal payload of PQON {}: {}: {}", bon.ret$PQON(), e.getClass().getSimpleName(), e.getMessage());
    ...
    /**
     * Performs a REST call and asserts that the result is an error (not HTTP 2xx).
    ...
    public static GenericResult performRESTCallAndAssertError(@Nonnull final RequestMethod method, @Nonnull final RESTConnection connection, @Nonnull final String path,
      @Nullable final Object payload, final int expectedError, @Nonnull final String message) {
        try {
            ...
            // check for missing response (for example in case of http 404, there is no response at all)
            if (result.responsePayload() == null) {
                LOGGER.error("{} returned HTTP {} and no response payload at all", summary, result.httpStatusCode());
                throw new RuntimeException("No result payload");
            }
    ...
    /**
     * Performs a REST call and asserts that the result is of a specific HTTP status code.
     * Throws an exception in case of a different status than expected.
     */
    public static void performRESTCallAndAssertHttpStatusCode(@Nonnull final RequestMethod method, @Nonnull final RESTConnection connection, @Nonnull final String path,
      @Nullable final Object payload, final int expectedHttpStatusCode, @Nonnull final String message) throws Exception {
        final ByteArray binaryPayload = toByteArray(payload, connection);
        final RESTResult result = connection.doIO(path, binaryPayload, method);
        if (result.httpStatusCode() == expectedHttpStatusCode) {
            return;
        }
        final String summary = getSummary(method, path, payload);
        LOGGER.error("{} returned HTTP status code {}, but I expected {}: {}", summary, result.httpStatusCode(), expectedHttpStatusCode, message);
        throw new RuntimeException(summary + " returned HTTP status " + result.httpStatusCode() + ", but I expected " + expectedHttpStatusCode + ": " + message);
    }
}
