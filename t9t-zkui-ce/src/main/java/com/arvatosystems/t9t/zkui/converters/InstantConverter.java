 ...
 * Regular Instant fields that render by field28, cell28 will never use it.
 */
public class InstantConverter implements Converter<Date, Instant, Component> {
     ...
     */
    @Override
    public Date coerceToUi(Instant val, Component comp, BindContext ctx) {
        if (val == null) {
     ...
     */
    @Override
    public final Instant coerceToBean(Date val, Component comp, BindContext ctx) {
        if (val == null) {
            return null; // do nothing
        }
        return val.toInstant();
    }
}
