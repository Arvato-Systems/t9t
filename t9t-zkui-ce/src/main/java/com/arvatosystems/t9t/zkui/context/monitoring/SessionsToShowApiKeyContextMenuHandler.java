...
@Singleton
@Named("sessions.ctx.apikey")
public class SessionsToShowApiKeyContextMenuHandler implements IGridContextMenu<SessionDTO> {
    protected final IT9tMessagingDAO messagingDAO = Jdp.getRequired(IT9tMessagingDAO.class);

    @Override
    public boolean isEnabled(final DataWithTracking<SessionDTO, TrackingBase> dwt) {
        final SessionDTO dto = dwt.getData();
        return dto.getApiKeyRef() != null;
    }

    @Override
    public void selected(final Grid28 lb, final DataWithTracking<SessionDTO, TrackingBase> dwt) {
        final SessionDTO dto = dwt.getData();
        JumpTool.jump("screens/user_admin/apikey28.zul", "objectRef", dto.getApiKeyRef().getObjectRef(), "screens/monitoring/sessions28.zul");
    }
}
