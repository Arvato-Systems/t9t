...
/** Shows messages with the same technical reference. */
@Singleton
@Named("asyncMessage.ctx.showRelated")
public class AsyncMessageShowRelatedContextHandler implements IGridContextMenu<AsyncMessageDTO> {

    @Override
    public boolean isEnabled(final DataWithTracking<AsyncMessageDTO, TrackingBase> dwt) {
        return dwt.getData().getRef() != null;
    }

    @Override
    public void selected(final Grid28 lb, final DataWithTracking<AsyncMessageDTO, TrackingBase> dwt) {
        JumpTool.jump("screens/monitoring/asyncMessage28.zul", "ref", dwt.getData().getRef(), "screens/monitoring/asyncMessage28.zul");
    }
}
