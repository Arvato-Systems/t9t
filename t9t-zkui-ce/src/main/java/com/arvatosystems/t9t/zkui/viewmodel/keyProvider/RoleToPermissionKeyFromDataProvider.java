...
public class RoleToPermissionKeyFromDataProvider implements IKeyFromDataProvider<RoleToPermissionDTO, TrackingBase> {
    ...
    public SearchFilter getFilterForKey(DataWithTracking<RoleToPermissionDTO, TrackingBase> dwt) {
        final StringFilter tenantFilter = SearchFilters.equalsFilter(T9tConstants.TENANT_ID_FIELD_NAME, ((DataWithTrackingS<RoleToPermissionDTO, TrackingBase>)dwt).getTenantId());
        final LongFilter roleFilter = SearchFilters.equalsFilter("roleRef", dwt.getData().getRoleRef().getObjectRef());
        final StringFilter permissionFilter = SearchFilters.equalsFilter("permissionId", dwt.getData().getPermissionId());
