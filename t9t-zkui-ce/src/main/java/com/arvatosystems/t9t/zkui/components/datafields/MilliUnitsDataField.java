...
public class MilliUnitsDataField extends AbstractDataField<MilliUnitsBox, MilliUnits> {
    ...
    public MilliUnitsDataField(DataFieldParameters params) {
        super(params);
    ...
    @Override
    public MilliUnitsBox getComponent() {
        return c;
    }

    @Override
    public void setValue(MilliUnits data) {
        c.setValue(data.toString());
    }

    @Override
    public MilliUnits getValue() {
        MilliUnits num = c.getValue();
        if (num == null)
            return null;
