...
@Singleton
@Named("requests.ctx.toChilds")
public class TransactionsToChildsContextHandler implements IGridContextMenu<MessageDTO> {

    @Override
    public void selected(final Grid28 lb, final DataWithTracking<MessageDTO, TrackingBase> dwt) {
        final MessageDTO dto = dwt.getData();
        JumpTool.jump("screens/monitoring/requests28.zul", "invokingProcessRef", dto.getObjectRef(), "screens/monitoring/requests28.zul");
    }
}
