...
public class AiDtoAssistCrudRequestHandler
    extends AbstractCrudSurrogateKeyRequestHandler<AiDtoAssistRef, AiDtoAssistDTO, FullTrackingWithVersion, AiDtoAssistCrudRequest, AiDtoAssistEntity> {
    ...
    @Override
    public CrudSurrogateKeyResponse<AiDtoAssistDTO, FullTrackingWithVersion> execute(final RequestContext ctx, final AiDtoAssistCrudRequest request) {
        final CrudSurrogateKeyResponse<AiDtoAssistDTO, FullTrackingWithVersion> resp = execute(ctx, mapper, resolver, request);
        return resp;
    }
}
