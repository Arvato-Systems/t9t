 ...
 */
package com.arvatosystems.t9t.annotations.jpa.active
...
class AutoMap28Processor extends AbstractClassProcessor {
    static final String REQUEST_PACKAGE_COMPONENT = "jpa.request"        // the piece after module...
    ...
    override doTransform(MutableClassDeclaration c, extension TransformationContext context) {
        ...
        for (r: c.declaredMethods) {
            if (r.parameters.size >= 2
                && r.parameters.get(0).type.isSimpleClass
                && r.parameters.get(1).type.isSimpleClass
                && r.returnType == primitiveVoid) {
                ...
                if (r.simpleName.startsWith("e2d")) {
                    ...
                    r.addError("Method of bad name: must be either e2d... or d2e..., found " + r.simpleName);
                    return;
                }
                ...
                c.addError("Classes annotated with @AutoMap may only contain methods of return type void with 2 parameters")
                return;
            }
...
//                        val servicePkgName = rqPkgName.substring(0, rqPkgName.length - 8) + "services"
                    ...
                    if (autoHandler.indexOf("R", lastDot+1) >= 0) {
                        ...
                        if (requestClassTypeRef !== null) {
                            createHandler(c, rqhPkgName, requestClassTypeRef, AbstractRequestHandler.newTypeReference(requestClassTypeRef), null, context) => [ m |
    ...
    def private MutableMethodDeclaration createHandler(MutableClassDeclaration c, String rqhPkgName, TypeReference rqClassRef, TypeReference parent,
        TypeReference mapperType, extension TransformationContext context) {
