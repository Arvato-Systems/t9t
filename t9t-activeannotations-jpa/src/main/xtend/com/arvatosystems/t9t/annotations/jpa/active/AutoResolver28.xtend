...
class AutoResolver28Processor extends AbstractClassProcessor {
    ...
    override doTransform(MutableClassDeclaration m, extension TransformationContext context) {
        ...
        for (r: m.declaredMethods) {
            ...
            if (myKeyType === null) {
                r.addError('''Entity type «entityClass.simpleName» must implement BonaKey<something>''')
                return
            }
            if (myTrackingType === null) {
                r.addError('''Entity type «entityClass.simpleName» must implement BonaTracking<something>''')
                return
            }
            ...
            if (r.simpleName.startsWith("get")) {
                ...
                val longKey = refType.isAssignableFrom(source.type)
                ...
                if (!bonaType.isAssignableFrom(source.type) && !stringKey) {
                    r.parameters.get(0).addError("First parameter of method named get... must be a BonaPortable or a String, found " + source.type)
                    return  // stop activity
                }
...
//                        addAnnotation(namedAnno).set("value", rtElement.simpleName.toFirstLower + "Resolver")
                ...
                if (longKey) {
                    ...
                    // add a static list which provides a default sort order
                ...
                val isExtraTenantRefRequired = r.parameters.filter[simpleName.equals(T9tConstants.TENANT_ID_FIELD_NAME)].empty
