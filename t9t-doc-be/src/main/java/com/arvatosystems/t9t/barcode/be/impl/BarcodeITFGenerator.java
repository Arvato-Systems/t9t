...
@Named("ITF")
@Singleton
public class BarcodeITFGenerator extends AbstractImageGenerator {
    @Override
    public MediaData generateImage(final String text, final ImageParameter params) throws Exception {
        final BitMatrix m = new ITFWriter().encode(text, BarcodeFormat.ITF, params.width, params.height);
