...
@Named("UPC_A")
@Singleton
public class BarcodeUPCAGenerator extends AbstractImageGenerator {
    @Override
    public MediaData generateImage(final String text, final ImageParameter params) throws Exception {
        final BitMatrix m = new UPCAWriter().encode(text, BarcodeFormat.UPC_A, params.width, params.height);
