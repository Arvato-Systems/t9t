...
@Named("AZTEC")
@Singleton
public class BarcodeAztecGenerator extends AbstractImageGenerator {
    @Override
    public MediaData generateImage(final String text, final ImageParameter params) throws Exception {
        final BitMatrix m = new AztecWriter().encode(text, BarcodeFormat.AZTEC, params.width, params.height);
