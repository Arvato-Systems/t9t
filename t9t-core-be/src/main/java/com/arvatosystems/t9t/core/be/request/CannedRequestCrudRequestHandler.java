...
public class CannedRequestCrudRequestHandler
    ...
    public ServiceResponse execute(final RequestContext ctx, final CannedRequestCrudRequest crudRequest) throws Exception {
        final CannedRequestDTO dto = crudRequest.getData();
        ...
        final CrudSurrogateKeyResponse<CannedRequestDTO, FullTrackingWithVersion> response = execute(ctx, crudRequest, resolver);
        if (Boolean.TRUE.equals(crudRequest.getSuppressResponseParameters()) && response.getData() != null) {
            response.getData().setRequest(null);
        }
        return response;
    }
}
