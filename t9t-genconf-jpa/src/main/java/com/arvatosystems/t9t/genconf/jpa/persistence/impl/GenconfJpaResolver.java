...
@Default
@Singleton
public class GenconfJpaResolver extends AbstractJpaResolver<ConfigRef, ConfigDTO, FullTrackingWithVersion, ConfigEntity> implements IConfigResolver {
    public GenconfJpaResolver() {
        super("Config", Jdp.getRequired(IConfigEntityResolver.class), Jdp.getRequired(IConfigDTOMapper.class));
    }

    @Override
    public ConfigRef createKey(final Long ref) {
        if (ref != null) {
            return new ConfigRef(ref);
        }
        return null;
    }
}
