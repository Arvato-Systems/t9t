...
@Default
@Singleton
public class LeanGridConfigJpaResolver extends AbstractJpaResolver<LeanGridConfigRef, LeanGridConfigDTO, FullTrackingWithVersion, LeanGridConfigEntity>
        implements ILeanGridConfigResolver {
    public LeanGridConfigJpaResolver() {
        super("LeanGridConfig", Jdp.getRequired(ILeanGridConfigEntityResolver.class), Jdp.getRequired(ILeanGridConfigDTOMapper.class));
    }

    @Override
    public LeanGridConfigRef createKey(final Long ref) {
        if (ref != null) {
            return new LeanGridConfigRef(ref);
        }
        return null;
    }
}
