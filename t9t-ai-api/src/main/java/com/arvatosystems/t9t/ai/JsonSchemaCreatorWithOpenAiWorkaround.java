...
public final class JsonSchemaCreatorWithOpenAiWorkaround {
     ...
     */
    public static JsonSchemaObject buildJsonSchemaObject(@Nullable final ClassDefinition metaData, @Nullable final String description, final boolean addPqon, final boolean isRequired) {
    ...
    private static AbstractJsonSchemaField buildField(final FieldDefinition metaData, final boolean addPqon) {
        ...
        case MAP:
            return buildJsonSchemaObject(null, comment, addPqon, metaData.getIsAggregateRequired()); // maps are represented as objects in JSON schema
        case SET:
            return new JsonSchemaSet(makeType(metaData.getIsAggregateRequired(), "array"), comment, true, buildFieldNoArray(metaData, null, addPqon));
        default:
    ...
    private static AbstractJsonSchemaField buildFieldNoArray(final FieldDefinition metaData, final String comment, final boolean addPqon) {
        ...
        case XENUMSET:
            ...
            return new JsonSchemaSet(makeType(required, "array"), comment, true, new JsonSchemaString("string", comment, null, MAX_ENUM_NAME_LENGTH, null, null));
        case OBJECT: {
            final ObjectReference objectReference = (ObjectReference) metaData;
            return buildJsonSchemaObject(objectReference.getLowerBound(), comment, addPqon, required);
        }
        case TEMPORAL: {
            final String bonaparteType = metaData.getBonaparteType().toLowerCase();
            switch (bonaparteType) {
