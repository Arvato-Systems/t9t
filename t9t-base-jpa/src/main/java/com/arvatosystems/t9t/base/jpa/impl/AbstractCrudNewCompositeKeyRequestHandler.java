...
> extends AbstractCrudAnyKeyRequestHandler<KEY, DTO, TRACKING, REQUEST, ENTITY> {
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractCrudNewCompositeKeyRequestHandler.class);

    // execute function of the interface description, but additional parameters
    // required in order to work around type erasure
    public NewCrudCompositeKeyResponse<REF, KEY, DTO, TRACKING> execute(final RequestContext ctx, final IEntityMapper<KEY, DTO, TRACKING, ENTITY> mapper,
            final IResolverNewCompositeKey<REF, KEY, TRACKING, ENTITY> resolver, final REQUEST crudRequest) {
