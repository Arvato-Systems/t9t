...
public class LazyJdbcSequenceBasedRefGenerator implements IRefGenerator {
    ...
    public LazyJdbcSequenceBasedRefGenerator() {
        ...
        for (int i = 0; i < NUM_SEQUENCES; ++i) {
            generatorTab[i] = new LazyJdbcSequenceBasedSingleRefGenerator(i, dialect, cacheSize);
        }
        for (int i = 0; i < NUM_SEQUENCES_UNSCALED; ++i) {
            generatorTab50xx[i] = new LazyJdbcSequenceBasedSingleRefGenerator(5000 + i, dialect, cacheSizeUnscaled);
            generatorTab60xx[i] = new LazyJdbcSequenceBasedSingleRefGenerator(6000 + i, dialect, cacheSizeUnscaled);
            generatorTab70xx[i] = new LazyJdbcSequenceBasedSingleRefGenerator(7000 + i, dialect, cacheSizeUnscaled);
        }
    }
