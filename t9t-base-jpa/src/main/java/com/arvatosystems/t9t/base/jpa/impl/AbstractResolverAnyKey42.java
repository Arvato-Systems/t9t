...
> implements IResolverAnyKey42<KEY, TRACKING, ENTITY> {
     ...
     */
    @Override
    public boolean readAllowed(final Long tenantRef) {
        if (isTenantIsolated()) {
            final Long myTenant = getSharedTenantRef();
     ...
     */
    @Override
    public boolean writeAllowed(final Long tenantRef) {
        final Long myTenant = getSharedTenantRef();
        if (myTenant.equals(tenantRef)) {
            return true; // always access to own tenant allowed
        }
