...
> extends AbstractCrudAnyKeyRequestHandler<KEY, DTO, TRACKING, REQUEST, ENTITY> {
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractCrudCompositeKeyRequestHandler.class);
    ...
    public CrudCompositeKeyResponse<KEY, DTO, TRACKING> execute(final RequestContext ctx, final IEntityMapper<KEY, DTO, TRACKING, ENTITY> mapper,
            final IResolverCompositeKey<KEY, KEY, TRACKING, ENTITY> resolver, final REQUEST crudRequest) {
     ...
     * @return
     */
    protected boolean recordExists(final KEY key, final IResolverCompositeKey<KEY, KEY, TRACKING, ENTITY> resolver) {
        try {
