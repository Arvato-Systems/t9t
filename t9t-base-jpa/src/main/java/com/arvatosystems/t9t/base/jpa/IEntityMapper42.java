 ...
 */
public interface IEntityMapper42<KEY extends Serializable, DTO extends BonaPortable, TRACKING extends TrackingBase, ENTITY extends BonaPersistableKey<KEY> & BonaPersistableTracking<TRACKING>> {
    ...
    /**
     * Maps a collection of entities to a list of standard DTOs.
     * This implementation allows to hand over a cache and a specific access graph.
     * The cache is indexed by target DTO simple name, its values are maps from objectRef to mapped DTO.
     * The graph is indexed by path of the field name (same as in the UI grid config JSON files), and the values are the PQONs of the desired DTO, or "null" to skip that field completely.
     */
